...
function smap_do_action(name) {
  if (SMAP_Actions[name])
    for (var i = 0; i < SMAP_Actions[name].length; i++) SMAP_Actions[name][i]();
}

function smap_add_action(name, cb) {
  if (!SMAP_Actions[name]) SMAP_Actions[name] = [];
...
function smap_add_filter(name, cb) {
  if (!SMAP_Filters[name]) SMAP_Filters[name] = [];
