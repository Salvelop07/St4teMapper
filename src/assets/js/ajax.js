  ...
  return $.ajax({
    ...
    success: function (data) {
      ...
      if (data.reload) location.reload();
        ...
        if (successCb) successCb(data, data && data.success);
        if ((!data || !data.success) && errorCb) errorCb(data);
      }
    },
    error: function (data) {
...
$(document).ready(function () {
  ...
  $("body").on("submit", "form.ajax-form", function (e) {
    ...
    if (!btn.data("smap-olabel")) btn.data("smap-olabel", btn.val());

    if (btn.data("smap-loading-label")) btn.val(btn.data("smap-loading-label"));

    smapAjax(related.action, { fields: fields }, function (data, success) {
      if (!success) btn.val(btn.data("smap-olabel"));
    });
