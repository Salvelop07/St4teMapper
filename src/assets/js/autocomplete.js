...
smap_add_action("load", function () {
  ...
  sugg.input.on("keypress keyup change focus", function (e) {
    if ($.inArray(e.type, ["keypress", "keyup"]) >= 0) {
      switch (e.which) {
        case 13: // enter
          if (e.type == "keypress") searchSend(e.ctrlKey);
          return false;
        case 27: // escape
          sugg.input.blur();
          return false;
  ...
  function suggUpdate() {
    if (!sugg.input.is(":focus")) closeSearch();
    else {
      var nval = $.trim(sugg.input.val());
      if (!sugg.last || sugg.last != nval || !sugg.open) {
        if (nval != "") openSearch(nval);
        else openSearchIntro();
  ...
  $(window).on("resize", function () {
    if (suggResizeTo) clearTimeout(suggResizeTo);
  ...
  function suggResize() {
    ...
    if (sugg.inner && sugg.open) {
      ...
      var bottom =
        sugg.wrap.offset().top + (sugg.more ? sugg.more.height() : 0);

      SMAP.acMinHeight = 1;
      var suggs = sugg.inner.find(".sugg-none").add(sugg.suggs);
      for (var i = 0; i < SMAP.acMinHeightSuggs; i++)
        if (i < suggs.length) SMAP.acMinHeight += suggs.eq(i).height();

      sugg.wrap.hide();
      var maxh = $("body").height(); //$(window).height() + $(window).scrollTop();
      sugg.inner.css({
        "max-height": Math.max(SMAP.acMinHeight, maxh - bottom - 30),
      });
      sugg.wrap.show();
    }
  }
  ...
  function openSearch(nval) {
    if ($("body").hasClass("search-sending")) return;
    var loading =
      sugg.input_opts &&
      (sugg.input_opts.loading || sugg.input_opts.loading === false) &&
      sugg.suggs &&
      sugg.suggs.length
        ? sugg.input_opts.loading
        : SMAP.loading;
    ...
    $("body")
      .off("click.smapSeachSugg")
      .on("click.smapSeachSugg", function (e) {
        if (!$(e.target).closest(".search-suggs, #search-input").length)
          closeSearch();
      });

    if (sugg.cache[nval]) appendResults(sugg.cache[nval]);
    else {
      (function (query) {
        smapAjax("search", { query: query }, function (data) {
          ...
          if (sugg.last == query) appendResults(data);
        });
      })(nval);
    }
    ...
    function appendResults(data) {
      ...
      sugg.inner = sugg.wrap
        .find(".search-suggs-results-inner")
        .html(data.results);
  ...
  sugg.wrap.on("mouseover", ".sugg > a", function (e) {
    if (!sugg.keymoving) suggSelect($(this), e);
  });
  ...
  function suggHook() {
    ...
    $(document).on("keypress.search-input keydown.search-input", function (e) {
      ...
      $("body")
        .off("mousemove.smap_suggs")
        .on("mousemove.smap_suggs", function () {
      ...
      switch (e.which) {
        ...
        case 33: // page up
        case 34: // page down
          if ($.inArray(e.which, [33, 34]) >= 0) count = 10;
          ...
          if (sugg.suggs) {
            ...
            if (count) {
              ...
              for (var i = 0; i < count; i++) {
                cnext = next
                  ? next[down ? "next" : "prev"]()
                  : sugg.suggs[down ? "first" : "last"]();
                if (!cnext.length) {
                  if (count == 1) next = cnext;
                  break;
                }
                next = cnext;
              }
            } else next = sugg.suggs[down ? "last" : "first"]();
            suggSelect(next, e);
          }
          return false;
  ...
  function searchSend(new_tab) {
    ...
    if (sugg.active) {
      ...
      if (v != "" || !$("body").hasClass("root")) {
        // do not send empty queries from homepage
        url = SMAP.searchUrl.replace("%s", encodeURIComponent(v));

        // replace or add &q=
        url = url.replace(/(([&\?])[^&]+=)(&(.*))?$/, "$2$4");
        url = url.replace(/[&\?]?[&\?]$/, "");
      } else sugg.input.focus();
    }
    if (url) {
  ...
  function suggSelect(li, e) {
    ...
    li = li.closest("div");
    if (sugg.active) {
      if (sugg.active.get(0) === li.get(0)) return;
      sugg.active.removeClass("sugg-active-active");
    }
    ...
    if (top < 0)
      scrollElt
        .stop()
        .animate({ scrollTop: scrollElt.scrollTop() + top - 30 }, "fast");
    else if (top + h > sugg.inner.height())
      scrollElt.stop().animate(
        {
          scrollTop: scrollElt.scrollTop() + top + h - sugg.inner.height() + 30,
        },
        "fast"
      );
  }
