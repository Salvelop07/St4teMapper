...
$(document).ready(function () {
  ...
  $(".header-title-menued").on("click", function () {
    ...
    if (
      $(".header-title-menu .menu-wrap")
        .toggleClass("menu-on")
        .hasClass("menu-on")
    )
      setTimeout(function () {
        $("body")
          .off("click.header-title-menu")
          .on("click.header-title-menu", function (e) {
  ...
  function update_iframe() {
    if (iframe_to) clearTimeout(iframe_to);
...
$.smapNiceAlert = function (opts) {
  var a = $(
    '<div class="nice-alert nice-alert-id-' +
      opts.id +
...
function smap_check_nice_alerts() {
  var a = null;
  while ((a = SMAP.nice_alerts.shift())) $.smapNiceAlert(a);
}
