...
smap_add_action("loaded", function () {
  ...
  dateInput.on("keypress keydown blur", function (e) {
    if (e.type == "blur") {
      if (!$(e.target).closest(".header-calendar").length) dateReset();
    } else if (e.which == 13)
      // enter
      dateChange();
  });
  ...
  $(".header-date").on("click", function () {
    ...
    if (
      $("body")
        .toggleClass("header-date-menu-on")
        .hasClass("header-date-menu-on")
    )
      setTimeout(function () {
        $("body")
          .off("click.header-top-date")
          .on("click.header-top-date", function (e) {
...
function smapDateIsValid(input) {
  // mysql format
  var bits = input.split("-");
  var d = new Date(bits[0], bits[1] - 1, bits[2]);
  return (
    d.getFullYear() == bits[0] &&
    d.getMonth() + 1 == bits[1] &&
