...
smap_add_action("load", function () {
  ...
  if (l.length) infinite_check();

  $(window)
    .off("scroll.smap-scroll")
    .on("scroll.smap-scroll", function () {
      infinite_check();
    });
  ...
  $("a.infinite-loader")
    .off("click.smap_infinite")
    .on("click.smap_infinite", function () {
  ...
  function load(t, opts) {
    ...
    if (wrap.filter(".loading").length) return;
      ...
      function (data, success) {
        ...
        if (success) {
          ...
          update = update.add(
            wrap
              .closest(".search-wrap")
              .find(".search-results-intro")
              .html(data.resultsLabel)
          );
          ...
          if (!l.length) $("body").off("scroll.smap-scroll");
        }
        if (!loading && done) {
          lto = null;
          infinite_check();
