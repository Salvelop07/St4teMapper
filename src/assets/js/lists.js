...
smap_add_action("load", function (elt) {
  $(".lists")
    .on("click", ".list-new-button", function () {
      ...
      if (user_input)
        smapAjax(
          ...
          function (data, success) {
            if (success) smapUpdate($(data.list_html).insertBefore(wrap));
          }
        );
      return false;
    })
    .on("click", ".list-rename", function () {
      ...
      if (user_input) {
        ...
        smapAjax("rename_list", args, function (data, success) {
          if (success)
            t.closest(".list")
              .smapSet({ list_title: data.list_title })
              .find(".list-title span")
              .html(data.list_title);
        });
      }
      return false;
    })
  ...
  $(".sheet").on("click", ".listed-lists-wrap > span .delete", function () {
    ...
    smapAjax(
      "set_entity_for_list",
      $.extend(true, {}, { active: false }, related),
      function (data, success) {
        if (success)
...
function autoaction_add_to_list(a, related) {
  ...
  if (!topWrap.length) topWrap = wrap;
  ...
  if (wrap.is(".loaded")) {
      ...
      if (e.which == 13) {
        ...
        if (user_input != "") {
          ...
          smapAjax(
            "autoaction",
            {
              related: $.extend(true, {}, { user_input: user_input }, related),
            },
            function (data, success) {
              if (success) {
                if (context == "sheet")
                  $(data.list_html_sheet).appendTo(
                    topWrap.find(".lists-list-holder")
                  );
                else {
                  if (data.is_new)
                    var li = $(data.list_html).prependTo(
                      topWrap.find(".lists-list-holder")
                    );
                  else var li = topWrap.find("li.list-" + data.list.list_id);
                  li.addClass("menu-item-active").addClass(
                    "menu-item-activated"
                  );
                }
              }
    ...
    lists.on("smap_menu_item_changed", function (e) {
      ...
      if (related.ajax_save)
        smapAjax(
          related.ajax_save,
          $.extend(true, {}, { active: active }, related),
          function (data, success) {}
        );
      //console.log(t, active, related);
    });

    if (context == "sheet") lists.appendTo(wrap).show();
    else lists.appendTo(wrap.empty().show()).slideDown("fast");
