...
smap_add_action("loaded", function () {
    ...
    smapAjax("live", { lives: lives }, function (data, success) {
      if (success) {
        ...
        for (var i = 0; i < data.result.length; i++) {
          ...
          } else live.replaceWith($(data.result[i]));
        }
        if (update.length) smapUpdate(update);
      }
    });
  ...
  if ($(".smap-disksize-fetchnow").length) disksizeUpdate();
        ...
        function (data) {
          if (data.success)
            for (var i in data.sizes)
              if (data.sizes[i])
                $(".smap-disksize-" + i)
                  .removeClass("smap-disksize-fetchnow")
                  .html(data.sizes[i]);
        },
        null,
        function () {
