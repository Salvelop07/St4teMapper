...
$(document).ready(function () {
    ...
    if (!m.hasClass("menu-on")) {
      ...
      setTimeout(function () {
        $("body").on("click.smapMenu", function (e) {
          if (
            $(e.target).closest(".menu").get(0) !== m.get(0) ||
            $(e.target).hasClass("menu-wrap")
          ) {
            closeMenu(m);
            e.stopPropagation();
            e.preventDefault();
  ...
  $("body").on("click", ".multisel-cb, .full-cb-menu li", function (e) {
    ...
    if (active) item.addClass("menu-item-activated");

    var m = item.closest(".menu").addClass("menu-changed");
    m.find(".menu-item-blank")[
      m.find(".menu-item-active").length ? "removeClass" : "addClass"
    ]("menu-item-active");
  ...
  $("body").on("click smap_enter", ".multisel a", function (e) {
    ...
    var isBlank =
      e.type != "smap_enter" && $(this).closest(".menu-item-blank").length;

    if (
      (e.type != "click" || active.length > 1) &&
      (e.type != "click" || (!e.ctrlKey && !e.shiftKey)) &&
      !isBlank
    ) {
      if (e.type != "smap_enter") {
        var t = $(this).closest("a").parent();
        if (
          !t.hasClass("menu-item-active") ||
          !t.hasClass("menu-item-activated")
        ) {
          t.toggleClass("menu-item-active");
          active = $(this)
            .closest(".menu-wrap.multisel")
            .find(".menu-item-active:not(.menu-item-blank) a");
        }
      }
      if (!active.length)
        active = $(this)
          .closest(".menu-wrap.multisel")
          .find(".menu-item-active a");

      // factorize URL parameters from active checkboxes
      var hrefs = [];
      var base = null;
      active.each(function () {
        if (!base) base = this.href.substr(0, this.href.indexOf("?"));
      ...
      for (var i = 0; i < hrefs.length; i++)
        for (var k in hrefs[i]) {
          if (typeof args[k] == "undefined") args[k] = [];
          val = decodeURIComponent(hrefs[i][k]);
          if ($.inArray(val, args[k]) < 0) args[k].push(val);
        }
      var url = [];
      for (var k in args)
  ...
  $(document).on("keypress keyup", function (e) {
    if (e.which == 13 && $(".menu-on .multisel").length) {
      // enter
      e.stopPropagation();
