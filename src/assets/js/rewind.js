...
$(document).ready(function () {
  ...
  function updateYear(manual) {
    if (refreshing) {
      if (manual) refreshing.abort();
      else return;
    }

    if (manual) $(".map-fetched .map-table").animate({ opacity: 0.4 });

    refreshing = smapAjax(
      "refresh_map",
      {
        current_year: year,
        extract: $(".spider-ctrl-extract-button input").is(":checked"),
      },
      function (data) {
  ...
  $("#wrap").on("click", ".spider-button", function (e) {
    var t = $(this);
    var turnOn = !t.filter(".spider-status-waiting, .spider-status-active")
      .length;

    smapAjax(
      "spider_power",
      { turnOn: turnOn, schema: t.data("schema") },
  ...
  $("#wrap").on("change", ".spider-ctrl-extract-button input", function (e) {
    ...
    smapAjax(
      "spiderExtract",
      { turnOn: turnOn, schema: $(".spider-button").data("schema") },
      function (data, success) {
        if (success) updateYear(true);
      }
    );
  });
