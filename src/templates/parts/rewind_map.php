...
$workers_count = 0;
$countPerYear = array();
$workersHtml = is_admin() ? get_workers_stats($workers_count, null, $countPerYear) : '';

$spider = array(
	'started' => !!$workers_count
);
...
$vars += get_spider_config($smap['query']['schema']) + array(
	'current_year' => isset($query['year']) ? $query['year'] : date('Y'),
);

$current_year = $vars['current_year'];
$mode = $vars['extract'] ? 'extract' : 'fetch';

$dateHeader = date('Y-m', strtotime($current_year.'-01-01'));

echo '<div class="map-years">';

$labels = get_bulletin_statuses();

$minYear = intval(date('Y', strtotime($vars['date_back'])));
...
for ($cyear = intval(date('Y')); true; $cyear--){
	...
	?><div class="map-year">
		<a href="<?= url(array('year' => $cyear, 'schema' => $query['schema']), 'rewind') ?>" class="map-year-inner <?php if ($cyear == $current_year) echo 'smap-year-current'; ?>">
			<?= $workersInd ?>
			...
			<span class="map-year-stats">
				<?php 
					foreach ($labels as $status => $c){
						...
						$formattedCount = is_numeric($count) ? format_sci($count, false) : $count;
						?>
...
$finalDate = min(strtotime(($current_year+1).'-01-01'), strtotime(date('Y-m-d')));
...
foreach (query('
	...
	GROUP BY b.date
	ORDER BY b.date ASC
', array($smap['query']['schema'], $current_year.'-01-01', date('Y-m-d', $finalDate))) as $c)
	$dbstats[$c['date']] = $c;
	
$workers = array();
foreach (query('SELECT w.pid, w.date FROM workers AS w LEFT JOIN spiders AS s ON w.spider_id = s.id WHERE s.bulletin_schema = %s AND w.date >= %s AND w.date < %s', array($smap['query']['schema'], $current_year.'-01-01', date('Y-m-d', $finalDate))) as $w)
...
for ($date = strtotime($current_year.'-01-01'); $date < $finalDate; $date = add_month($date)){
...
//echo '<h3>'.sprintf(_('Year %s'), $current_year).'</h3>';
?>
